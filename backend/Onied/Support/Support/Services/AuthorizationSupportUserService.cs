    ...
    ) : IAuthorizationSupportUserService
{
    public async Task<SupportUser> AuthorizeSupportUser(Guid? userId)
    {
        ...
        {
            logger.LogWarning("User authorization failed for userId: {UserId}. The user is not a support user", userId);
            throw new ForbidException("The user with the given userId is not a support user");
        }
